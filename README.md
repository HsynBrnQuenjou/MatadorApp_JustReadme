...
1. **Flutter SDK** (3.6.0 veya üzeri)
   ```bash
   flutter --version
   ```
...
1. **Projeyi Klonlayın**
   ```bash
   git clone https://github.com/HsynBrnQuenjou/matador_app.git
   cd matador_app
   ```
...
3. **Asset'leri Kontrol Edin**
   ```bash
   # pubspec.yaml dosyasında tüm asset'lerin tanımlı olduğundan emin olun
...
**Problem**: Flutter sürüm uyuşmazlığı
```bash
flutter upgrade
flutter doctor -v
```

**Problem**: Asset yüklenmiyor
```bash
flutter clean
flutter pub get
flutter run
...
<div align="center">
  ...
  <br>
  <img src="matador_app/assets/logo4.png" alt="BARIŞ YAYINLARI" width="200">
</div>
